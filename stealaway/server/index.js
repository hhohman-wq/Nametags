...
import {
  affiliateTarget, hotelTarget, hotelRate, recordClick, createCheckout, handleStripeWebhook,
  adminStats, FREE_WATCH_LIMIT, PRO_EARLY_ACCESS_MIN, PRO_PRICE_CENTS
} from './revenue.js';

const DAY = 86400000;
const DEFAULT_WINDOW_DAYS = 14; // Stealaway leads with trips leaving soon

function isWeekendDepart(dateStr) {
  const d = new Date(dateStr + 'T12:00:00Z').getUTCDay();
  return d === 5 || d === 6 || d === 0; // Fri, Sat, Sun
}
function passesWindow(dateStr, win, now) {
  const t = new Date(dateStr + 'T12:00:00Z').getTime();
  if (t < now - 12 * 3600000) return false; // never surface departed trips
  if (win === 'weekend') return t <= now + 10 * DAY && isWeekendDepart(dateStr);
  if (win > 0) return t <= now + win * DAY;
  return true;
}
// Leaving-soon boost: the closer the departure, the higher it ranks.
function urgencyBoost(dateStr, now) {
  const days = (new Date(dateStr + 'T12:00:00Z').getTime() - now) / DAY;
  return days <= 3 ? 35 : days <= 7 ? 28 : days <= 14 ? 18 : 0;
}
...
export async function buildApp({ dbPath, db = openDb(dbPath), providers = createProviders(), autoSeed = true } = {}) {
  ...
  const destStmt = db.prepare('SELECT code, city, country, vibes, nightly, todo FROM airports WHERE code = ?');
  ...
  function cardBase(routeId, origin, destCode) {
    ...
    return {
      routeId,
      origin,
      dest: { ...dest, vibes: JSON.parse(dest.vibes), todo: JSON.parse(dest.todo ?? '[]') },
      trend: trendStmt.all(routeId).map((r) => r.price)
    };
  }

  // Book/Stay links route through /go so every outbound click is recorded.
  const goUrl = (c, userId, type = 'flight') => {
    const p = new URLSearchParams({
      route: c.routeId, depart: c.departDate, ret: c.returnDate, type,
      price: type === 'hotel' ? c.stayTotal : c.price
    });
  ...
  // Whole-trip estimate: flight + nights at the destination's typical rate.
  function withTrip(card, userId) {
    const nights = Math.max(1, Math.round(
      (new Date(card.returnDate + 'T12:00:00Z') - new Date(card.departDate + 'T12:00:00Z')) / DAY
    ));
    if (card.dest.nightly) {
      card.nights = nights;
      card.stayNightly = Math.round(card.dest.nightly);
      card.stayTotal = Math.round(nights * card.dest.nightly);
      card.tripTotal = card.price + card.stayTotal;
      card.stayUrl = goUrl(card, userId, 'hotel');
    }
    card.bookUrl = goUrl(card, userId, 'flight');
    return card;
  }

  function getFeed({ userId, win = DEFAULT_WINDOW_DAYS, vibe = null }) {
    ...
    for (const d of dealRows) {
      if (!passesWindow(d.depart_date, win, now)) continue;
      ...
      const card = {
        ...
        score: scoreDeal(d, { budget, vibes: userVibes, now }) + urgencyBoost(d.depart_date, now)
      };
      cards.push(withTrip(card, userId));
    }
    ...
    for (const f of latestFares) {
      ...
      if (!passesWindow(f.depart_date, win, now)) continue;
      ...
      const card = {
        ...
        returnDate: ret,
        expiresAt: null,
        headline: `Quick trip: ${dest.city}, leaving soon`,
        watched: watchedRoutes.has(f.route_id),
        score: 30 + Math.max(0, pctBelow) * 1.5 + urgencyBoost(f.depart_date, now)
      };
      cards.push(withTrip(card, userId));
    }
  ...
  const routesTable = {
    'GET /api/health': () => ({ ok: true, name: 'stealaway', providers: providers.map((p) => p.name) }),
    ...
    'GET /api/feed': (_req, url) =>
      getFeed({
        ...
        win: parseWindow(url.searchParams.get('window')),
    ...
    'POST /api/watches': async (req) => {
      ...
      if (user.pro !== 1 && !exists && count >= FREE_WATCH_LIMIT) {
        throw Object.assign(
          new Error(`Free plan watches ${FREE_WATCH_LIMIT} trips. Stealaway Pro watches as many as you like.`),
    ...
    'POST /api/billing/checkout': async (req) => {
      ...
      const origin = process.env.STEALAWAY_PUBLIC_URL || `http://localhost:${process.env.PORT || 4600}`;
    ...
    'GET /api/admin/stats': (_req, url) => {
      const key = process.env.STEALAWAY_ADMIN_KEY;
  ...
  async function handle(req, res) {
    ...
    try {
      ...
      // Tracked booking handoff (flight or stay): record the click, redirect out.
      if (req.method === 'GET' && url.pathname === '/go') {
        const routeId = Number(url.searchParams.get('route'));
        const route = db
          .prepare('SELECT r.id, r.origin, r.dest, a.city AS dest_city FROM routes r JOIN airports a ON a.code = r.dest WHERE r.id = ?')
          .get(routeId);
        ...
        const isHotel = url.searchParams.get('type') === 'hotel';
        const target = isHotel
          ? hotelTarget({ city: route.dest_city })
          : affiliateTarget({ origin: route.origin, dest: route.dest, departDate, returnDate });
        recordClick(db, {
          userId: url.searchParams.get('user'),
          ...
          rate: isHotel ? hotelRate() : undefined
        });
...
<title>${esc(title)} — Stealaway</title>
<meta property="og:title" content="${esc(title)}">
<meta property="og:description" content="Departs ${esc(d.depart_date)}, back ${esc(d.return_date)}. Typically $${esc(d.typical)}. Caught on Stealaway.">
<link rel="icon" type="image/svg+xml" href="/favicon.svg">
<link rel="stylesheet" href="/styles.css"></head>
<body class="share-body"><main class="share-card">
<p class="share-brand">STEALAWAY</p>
...
if (isMain) {
  ...
  const stop = startPolling(db, providers, {
    intervalMs: Number(process.env.STEALAWAY_POLL_MS) || 60000,
    log: (m) => console.log(`[stealaway] ${m}`)
  });
  server.listen(port, () =>
    console.log(`[stealaway] serving on http://localhost:${port} (providers: ${providers.map((p) => p.name).join(', ')})`)
  );
