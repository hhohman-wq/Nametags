...
test('fresh rare fares are held back from free users, visible to Pro', () =>
  withServer(async (base, db) => {
    const user = await makeUser(base);
    // Plant a rare fare that landed one minute ago on a JFK route, leaving soon.
    const route = db.prepare("SELECT id FROM routes WHERE origin = 'JFK' LIMIT 1").get();
    const depart = new Date(Date.now() + 5 * 86400000).toISOString().slice(0, 10);
    const ret = new Date(Date.now() + 10 * 86400000).toISOString().slice(0, 10);
    db.prepare(
      `INSERT INTO deals (route_id, type, price, typical, pct_below, depart_date, return_date, headline, created_at, active)
       VALUES (?, 'anomaly', 199, 420, 53, ?, ?, 'test rare fare', ?, 1)`
    ).run(route.id, depart, ret, new Date(Date.now() - 60000).toISOString());
...
test('hotel handoff via /go: Hotellook when marker set, tracked at hotel rate', () =>
  withServer(async (base, db) => {
    const user = await makeUser(base);
    const res = await get(base, `/go?route=1&type=hotel&depart=2026-09-10&ret=2026-09-15&price=600&user=${user.id}`);
    assert.equal(res.status, 302);
    assert.ok(res.headers.get('location').includes('google.com/travel/hotels'), 'no marker in tests -> untagged hotels');
    const click = db.prepare("SELECT * FROM clicks WHERE target = 'google-hotels'").get();
    assert.equal(click.price, 600);
    assert.equal(click.est_commission, 0);
  }));
