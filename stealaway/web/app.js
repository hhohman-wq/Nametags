/* Stealaway feed client. State lives in localStorage under 'stealaway.user'. */
...
const state = {
  user: loadUser(),
  window: '14', // Stealaway defaults to trips leaving soon
  vibe: '',
  airports: null,
  watched: new Set()
};
...
function loadUser() {
  try { return JSON.parse(localStorage.getItem('stealaway.user')) || null; } catch { return null; }
}
function saveUser(u) {
  state.user = u;
  try { localStorage.setItem('stealaway.user', JSON.stringify(u)); } catch { /* private mode */ }
}
...
function renderCard(deal) {
  ...
  if (deal.tripTotal) {
    const trip = $('.trip-total', node);
    trip.hidden = false;
    trip.href = deal.stayUrl;
    $('.trip-total-label', trip).innerHTML =
      `Whole trip ≈ <strong></strong> · flight + ${deal.nights} nights (~$${deal.stayNightly}/nt)`;
    $('strong', trip).textContent = `$${deal.tripTotal}`;
  }
  if (deal.dest.todo && deal.dest.todo.length) {
    const todo = $('.todo-line', node);
    todo.hidden = false;
    todo.innerHTML = `<span class="todo-kicker">WHILE YOU'RE THERE</span>`;
    todo.append(deal.dest.todo.slice(0, 3).join(' · '));
  }
  ...
  $('.btn-share', node).addEventListener('click', async () => {
    ...
    const text = `${deal.origin} → ${deal.dest.city} for $${deal.price} (${deal.pctBelow}% below typical) on Stealaway`;
    if (navigator.share) {
      navigator.share({ title: 'Stealaway deal', text, url }).catch(() => {});
    } else {
...
wireChipRow('#windowChips', 'window', (v) => { state.window = v; loadFeed(); });
...
$('#proUpgrade').addEventListener('click', async () => {
  ...
  try {
    ...
    if (res.url) {
      ...
      toast('Stealaway Pro is active. Rare fares now land instantly.');
      await loadFeed();
    }
