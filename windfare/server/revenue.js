...
//   2. Windfare Pro ($49/yr) — Stripe Checkout when keys are configured, a
...
// Blended flight-affiliate commission estimate (Travelpayouts flights pay
// ~1.1–2.2% of ticket value). Estimates only — the network's dashboard is
// the source of truth for payouts.
const rate = (env = process.env) => Number(env.WINDFARE_COMMISSION_RATE) || 0.016;
...
export function recordClick(db, { userId, dealId, routeId, price, network, commissionable }, env = process.env) {
  const est = commissionable ? Math.round(price * rate(env) * 100) / 100 : 0;
  db.prepare(
