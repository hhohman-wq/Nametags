...
export function seed(db, { seedNum = 42, historyDays = 90, now = Date.now() } = {}) {
  ...
  const insAirport = db.prepare(
    'INSERT INTO airports (code, city, name, country, vibes, is_origin) VALUES (?, ?, ?, ?, ?, ?)'
  );
  for (const o of ORIGINS) insAirport.run(o.code, o.city, o.name, 'USA', '[]', 1);
  for (const d of DESTINATIONS) insAirport.run(d.code, d.city, null, d.country, JSON.stringify(d.vibes), 0);
  ...
  for (let pass = 0; pass < 6; pass++) {
    for (const r of routes) {
      dealsCreated += ingestQuotes(db, r, provider.quotes(r), { now }).length;
    }
  }
  expireDeals(db, { now });
